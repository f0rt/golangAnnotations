...
func TestCorrectOutboundtServiceAnnotation(t *testing.T) {
	...
	ann, ok := registry.ResolveAnnotationByName([]string{`// @OutboundClient( description = "my descr", external = "true", http = "true")`}, "OutboundClient")
	assert.True(t, ok)
...
func TestCorrectOutboundOperationAnnotation(t *testing.T) {
	...
	a, ok := registry.ResolveAnnotation(`// @OutboundOperation( description = "my descr", inbound = "true", outbound = "false" )`)
	assert.True(t, ok)
