...
func TestParseStructsInFile(t *testing.T) {
	...
	assertStruct(t,
		intermediatemodel.Struct{PackageName: "structs", Name: "Person", DocLines: []string{"// Struct comment before type"}},
...

	{
		...
		assertField(t,
			intermediatemodel.Field{Name: "FirstName", TypeName: "string"},
			s.Fields[0])

		assertField(t,
			intermediatemodel.Field{Name: "LastName", TypeName: "string"},
			s.Fields[1])

		assertField(t,
			intermediatemodel.Field{Name: "Age", TypeName: "int", CommentLines: []string{"// Age comment"}},
			s.Fields[2])

		assertField(t,
			intermediatemodel.Field{Name: "Nice", TypeName: "*bool", DocLines: []string{"// Before nice comment"}, CommentLines: []string{"// After Nice comment"}},
			s.Fields[3])

		assertField(t,
			intermediatemodel.Field{Name: "Color", TypeName: "ColorType", DocLines: []string{"// Before Color comment"}, Tag: "`json:\"COLOR_TYPE\"`"},
			s.Fields[4])

		assertField(t,
			intermediatemodel.Field{Name: "OptionalColor", TypeName: "*ColorType"},
			s.Fields[5])

		assertField(t,
			intermediatemodel.Field{Name: "Father", TypeName: "*Person"},
			s.Fields[6])

		assertField(t,
			intermediatemodel.Field{Name: "Uncles", TypeName: "[]*Person"},
			s.Fields[7])

		assertField(t,
			intermediatemodel.Field{Name: "Children", TypeName: "[]Person"},
			s.Fields[8])

		assertField(t,
			intermediatemodel.Field{Name: "ChildMap", TypeName: "map[string]Person"},
			s.Fields[9])

		assertField(t,
			intermediatemodel.Field{Name: "ChildPointerMap", TypeName: "map[string]*Person"},
			s.Fields[10])

		assertField(t,
			intermediatemodel.Field{Name: "ChildrenMap", TypeName: "map[string][]Person"},
			s.Fields[11])

		assertField(t,
			intermediatemodel.Field{Name: "ChildrenPointerMap", TypeName: "map[string][]*Person"},
			s.Fields[12])
	}
}
...
func assertStruct(t *testing.T, expected intermediatemodel.Struct, actual intermediatemodel.Struct) {
...
func assertField(t *testing.T, expected intermediatemodel.Field, actual intermediatemodel.Field) {
	//t.Logf("expected: %+v, actual: %+v", expected, actual)
	assertStringSlice(t, expected.DocLines, actual.DocLines)
