...

	{
...

		{
			assert.Len(t, i.Methods, 2)
			{
				...
				assertField(t, intermediatemodel.Field{Name: "c", TypeName: "context.Context"}, m.InputArgs[0])
				assertField(t, intermediatemodel.Field{Name: "req", TypeName: "Req"}, m.InputArgs[1])

				assert.Equal(t, 2, len(m.OutputArgs))
				assertField(t, intermediatemodel.Field{TypeName: "Resp"}, m.OutputArgs[0])
				assertField(t, intermediatemodel.Field{TypeName: "error"}, m.OutputArgs[1])
			}
			{
				m := i.Methods[1]
