...
func (p *myParser) ParseSourceDir(dirName string, includeRegex string, excludeRegex string) (intermediatemodel.ParsedSources, error) {
	...
	if err != nil {
		...
		return intermediatemodel.ParsedSources{}, err
	}
	...
	return intermediatemodel.ParsedSources{
		Structs:    v.Structs,
...
func parseSourceFile(srcFilename string) (intermediatemodel.ParsedSources, error) {
	if debugAstOfSources {
...
func embedOperationsInStructs(visitor *astVisitor) {
	mStructMap := make(map[string]*intermediatemodel.Struct)
...
type astVisitor struct {
	...
	Interfaces      []intermediatemodel.Interface
	Typedefs        []intermediatemodel.Typedef
...
func extractGenDeclForStruct(node ast.Node, imports map[string]string) *intermediatemodel.Struct {
	if genDecl, ok := node.(*ast.GenDecl); ok {
...
func extractSpecsForStruct(specs []ast.Spec, imports map[string]string) *intermediatemodel.Struct {
	if len(specs) >= 1 {
		if typeSpec, ok := specs[0].(*ast.TypeSpec); ok {
			if structType, ok := typeSpec.Type.(*ast.StructType); ok {
				return &intermediatemodel.Struct{
...
func extractGenDeclForTypedef(node ast.Node) *intermediatemodel.Typedef {
	if genDecl, ok := node.(*ast.GenDecl); ok {
...
func extractSpecsForTypedef(specs []ast.Spec) *intermediatemodel.Typedef {
	if len(specs) >= 1 {
		if typeSpec, ok := specs[0].(*ast.TypeSpec); ok {
			mTypedef := intermediatemodel.Typedef{
				Name: typeSpec.Name.Name,
			}
...
func extractGenDeclForEnum(node ast.Node) *intermediatemodel.Enum {
	if genDecl, ok := node.(*ast.GenDecl); ok {
...
func extractSpecsForEnum(specs []ast.Spec) *intermediatemodel.Enum {
	if typeName, ok := extractEnumTypeName(specs); ok {
		mEnum := intermediatemodel.Enum{
			Name:         typeName,
			EnumLiterals: []intermediatemodel.EnumLiteral{},
		}
		for _, spec := range specs {
			if valueSpec, ok := spec.(*ast.ValueSpec); ok {
				enumLiteral := intermediatemodel.EnumLiteral{
...
func extractInterface(node ast.Node, imports map[string]string) *intermediatemodel.Interface {
	if genDecl, ok := node.(*ast.GenDecl); ok {
...
func extractSpecsForInterface(specs []ast.Spec, imports map[string]string) *intermediatemodel.Interface {
	if len(specs) >= 1 {
		if typeSpec, ok := specs[0].(*ast.TypeSpec); ok {
			if interfaceType, ok := typeSpec.Type.(*ast.InterfaceType); ok {
				return &intermediatemodel.Interface{
					Name:    typeSpec.Name.Name,
...
func extractInterfaceMethods(fieldList *ast.FieldList, imports map[string]string) []intermediatemodel.Operation {
	methods := make([]intermediatemodel.Operation, 0)
	for _, field := range fieldList.List {
		if len(field.Names) > 0 {
			if funcType, ok := field.Type.(*ast.FuncType); ok {
				methods = append(methods, intermediatemodel.Operation{
...
func extractOperation(node ast.Node, imports map[string]string) *intermediatemodel.Operation {
	if funcDecl, ok := node.(*ast.FuncDecl); ok {
		mOperation := intermediatemodel.Operation{
